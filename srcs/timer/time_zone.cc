...
{
  vector<net::Transition> transitions;
  vector<net::Localtime> localtimes;
  vector<string> names;
  string abbreviation;
};
...
{
  if (!net::readTimeZoneFile(zonefile, data_.get()))
  {
    data_.reset();
  }
}
...
TimeZone::TimeZone(int eastOfUtc, const char* name)
  : data_(new TimeZone::Data)
{
  data_->localtimes.push_back(net::Localtime(eastOfUtc, false, 0));
...
struct tm TimeZone::toLocalTime(time_t seconds) const
{
  ...
  net::Transition sentry(seconds, 0, 0);
  const net::Localtime* local = findLocaltime(data, sentry, net::Comp(true));
...
time_t TimeZone::fromLocalTime(const struct tm& localTm) const
{
  ...
  net::Transition sentry(0, seconds, 0);
  const net::Localtime* local = findLocaltime(data, sentry, net::Comp(false));
  if (localTm.tm_isdst)
  {
